############################################
# Locals (naming convention: deathstar-*)
############################################
locals {
...
# Explanation: deathstar needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
...
# Explanation: deathstar refuses to carry static keys—this role lets EC2 assume permissions safely.
...
resource "aws_iam_role_policy_attachment" "deathstar_ec2_ssm_attach" {
...
resource "aws_iam_role_policy_attachment" "deathstar_ec2_cw_attach" {
  role      = aws_iam_role.deathstar_ec2_role01.name
...
resource "aws_instance" "deathstar_ec201" {
  ami                    = var.ec2_ami_id
...
# Explanation: Parameter Store is deathstar’s map—endpoints and config live here for fast recovery.
...
# resource "aws_vpc_endpoint" "deathstar_vpce_ssm" { ... }